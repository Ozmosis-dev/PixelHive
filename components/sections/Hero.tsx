  ...
  return (
    <section className="relative w-full bg-ph-black">
      ...
      <div className="relative mx-auto grid max-w-[1380px] grid-cols-[0.88fr_1.12fr] gap-x-[61.52px] px-6 pt-[39px] pb-[71.94px]">
        {/* Left column */}
        <div className="relative flex flex-col pt-[39px]">
          <p className="font-inter text-[12.5px] font-bold uppercase leading-[18.72px] tracking-[2.246px] text-ph-yellow-mid">
            PixelHive
          </p>

          <h1
            className="mt-[16px] max-w-[631.68px] font-manrope text-[96px] font-bold leading-[91.2px] tracking-[-2.88px] text-ph-zircon"
            style={{ textShadow: "0px 8px 30px rgba(0,0,0,0.22)" }}
          >
            Edit listings<br />at hive<br />speed
          </h1>

          <p className="mt-[35px] w-[559px] font-inter text-[18.5px] leading-[30.45px] text-ph-zircon/90">
            Upload, enhance, proof, and deliver property photos in one<br />
            workflow built for high-volume teams.
          </p>
          ...
          <div className="mt-[32px] flex flex-wrap items-center gap-[16px]">
            <a
              ...
              style={{
                background:
                  "linear-gradient(135.16deg, #F4B400 0%, #FFC247 100%)",
              }}
            >
              Explore Features
            </a>
            <a
              href="#"
              className="font-inter text-[14px] font-semibold leading-[21px] text-ph-zircon"
            >
              Browse the FAQ
            </a>
          </div>

          <div className="mt-[35.2px] flex w-[559px] flex-col gap-[16px]">
            {[
              { label: "The Hive", body: "Photographers, editors, and coordinators in one flow" },
              { label: "What's Buzzing", body: "Review states, feedback, and delivery status stay visible" },
            ].map((card) => (
              <div key={card.label} className="border-t border-white/10 pt-[16px]">
                <p className="font-inter text-[12.5px] uppercase leading-[18.72px] tracking-[1.498px] text-white/[0.72]">
                  {card.label}
                </p>
                <p className="mt-[4px] font-inter text-[15.7px] font-bold leading-[23.52px] text-white">
                  {card.body}
                </p>
              </div>
            ))}
          ...
          >
            ...
            <div className="flex items-center gap-[7.2px] px-4 pt-4">
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
                  className="block h-[9.91px] w-[9.91px] rounded-full bg-white/[0.18]"
                />
              ))}
            </div>

            {/* Inner grid: sidebar + main */}
            <div className="grid grid-cols-[192px_1fr] gap-4 p-4">
              ...
              <aside className="flex flex-col gap-[8.8px] rounded-[22px] border border-white/[0.08] bg-white/[0.03] px-[17px] pt-[17px] pb-[300px]">
                ...
                {["Uploads", "Sequence", "Edit", "QC", "Delivery"].map((item) => (
                  <div
                    key={item}
                    className="flex min-h-[41.6px] items-center rounded-[16px] bg-white/[0.04] px-[14.4px] py-[12px]"
                  >
              ...
              >
                ...
                {/* Body grid - flex for guaranteed bottom alignment */}
                <div className="flex h-[575px] gap-4">
                  {/* Image preview card - left */}
                  <article className="flex flex-1 flex-col gap-[12.8px] rounded-[20px] border border-white/[0.08] bg-white/[0.03] px-[17px] pt-[16px] pb-[17px]">
                    ...
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(180deg, rgba(143,207,255,0.52) 0%, rgba(248,203,145,0.18) 48%, rgba(29,23,19,0.72) 100%), linear-gradient(134deg, rgba(255,255,255,0.18), rgba(255,255,255,0))",
                      }}
                    >
                      <div
                        className="absolute"
                        style={{
                          ...
                          borderRadius: "16px",
                          background:
                            "linear-gradient(180deg, rgba(214,239,255,0.92) 0%, rgba(137,201,255,0.28) 100%), rgba(255,255,255,0.2)",
                        }}
                      />
                      <div
                        className="absolute bottom-0 left-0 right-0 h-[76.8px]"
                        style={{ background: "linear-gradient(180deg, rgba(73,46,29,0), rgba(73,46,29,0.92))" }}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3 pt-[2px]">
                      <p className="font-inter text-[13.1px] leading-[19.68px] text-white/[0.68]">
                        Exposure<br />balanced
                      </p>
                      <p className="font-inter text-[13.1px] leading-[19.68px] text-white/[0.68]">
                        Sky<br />preserved
                      </p>
                  ...
                  {/* Right column - 2 stacked panels filling full height */}
                  <div className="flex w-[228px] flex-col gap-4">
                    {/* Processing queue */}
                    <article className="flex flex-1 flex-col rounded-[20px] border border-white/[0.08] bg-white/[0.03] px-[17px] pt-[16px] pb-[17px]">
                      ...
                      {[
                        ...
                        <div
                          key={row.addr}
                          className={`flex flex-1 items-center justify-between ${i > 0 ? "border-t border-white/[0.08]" : ""}`}
                        >
                          <div>
